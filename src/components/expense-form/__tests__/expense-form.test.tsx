...
describe('components/expense-form/<ExpenseForm />', () => {
  ...
  it('renders component properly when is not editing', () => {
    ...
    expect(screen.queryByText('Invalid input values. Please check your entered data!')).toBeNull();
  ...
  it('calls inputChangeHandler when one of the inputs are typed', () => {
    ...
    fireEvent.changeText(screen.getByTestId('components.expense-form.components.input.text-input.amount'), '24')

    expect(mockUseExpenseFormValues.inputChangeHandler).toHaveBeenCalledTimes(1);
  })
  ...
  it('shows text when one of the fields is not valid', () => {
    mockUseExpenseForm.mockReturnValue({
      ...mockUseExpenseFormValues, inputValues: {
        ...mockUseExpenseFormValues.inputValues, amount: {
          value: '10',
          isValid: false
        }
      }
    ...
    expect(screen.getByText('Invalid input values. Please check your entered data!')).toBeTruthy();
